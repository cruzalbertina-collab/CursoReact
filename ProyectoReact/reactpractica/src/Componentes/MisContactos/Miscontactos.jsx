...
import './Miscontactos.css'
export default function MisContactos() {
  const [contactosAgregados, setContactos] = useState([]);
  ...
  const filtrados = contactosAgregados.filter((p) => {
  ...
  return (
    <div className="divContactos" >
      ...
      <label>Buscar por nombre</label><input onChange={(e) => setFiltro(e.target.value)} placeholder="Digite el nombre a buscar.."></input>
       <table className="contactos" border={1}>
        <thead>
          <tr>
            <th>Nombre</th>
            <th>Correo</th>
